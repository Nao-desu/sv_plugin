...
button = [[button_gen("单抽",'sv抽卡',enter=True),button_gen("十连",'sv十连',enter=True),button_gen("一井",'sv井',enter=True),button_gen('帮助','https://www.koharu.cn/docs/shadowverse/shadowverse.html#%E6%A8%A1%E6%8B%9F%E6%8A%BD%E5%8D%A1',type_int = 0)]]
...
async def gacha1(bot,ev):
    ...
    try:
        ...
        if is_rot:
            msg = generate_md(2,['抽卡结果',f'img#{size[0]}px #{size[1]}px',url,'',f'抽取卡包:溯时指定'],button)
        else:
            msg = generate_md(2,['抽卡结果',f'img#{size[0]}px #{size[1]}px',url,'',f'抽取卡包:{card_set[card_set_gacha]}'],button)
...
async def gacha10(bot,ev):
    ...
    try:
        ...
        if is_rot:
            msg = generate_md(2,['抽卡结果',f'img#{size[0]}px #{size[1]}px',url,msg,f'抽取卡包:溯时指定'],button)
        else:msg = generate_md(2,['抽卡结果',f'img#{size[0]}px #{size[1]}px',url,msg,f'抽取卡包:{card_set[card_set_gacha]}'],button)
        await bot.send(ev,msg)
    except Exception as e:
        await bot.send(ev,f'发送失败：{e}')
        traceback.print_exc()
